...
void City::bulldoze(const Tile &tile) {
	for (int pos = 0; pos < this->map.width * this->map.height; ++pos) {
		if (this->map.selected[pos] == 1) {
			if (this->map.tiles[pos].tileType == TileType::RESIDENTIAL) this->populationPool += this->map.tiles[pos].population;
			else if (this->map.tiles[pos].tileType == TileType::COMMERCIAL || this->map.tiles[pos].tileType == TileType::INDUSTRIAL) this->employmentPool += this->map.tiles[pos].population;
...
void City::tileChanged() {
	...
	this->map.findConnectedRegions({ TileType::ROAD, TileType::RESIDENTIAL, TileType::COMMERCIAL, TileType::INDUSTRIAL }, 0);
	return;
}
	...
	while (std::getline(inputFile, line)) {
		...
		if (std::getline(lineStream, key, '=')) {
			...
			if (std::getline(lineStream, value)) {
				...
				else std::cerr << "Error, no value for key " << key << std::endl;
			}
		}
	}
	inputFile.close();
...
void City::update(float dt) {
	...
	for (int i = 0; i < this->map.tiles.size(); ++i) {
		...
		if (tile.tileType == TileType::RESIDENTIAL) {
			...
			if (rand() % 100 < 15 * (1.0 - this->commercialTax)) this->distributePool(this->employmentPool, tile, 0.0);
		} else if (tile.tileType == TileType::INDUSTRIAL) {
			if (this->map.resources[i] > 0 && rand() % 100 < this->population) {
				++tile.production;
				--this->map.resources[i];
			}
			if (rand() % 100 < 15 * (1.0 - this->industrialTax)) this->distributePool(this->employmentPool, tile, 0.0);
		}
		tile.update();
	}
			...
			for (auto &tile2 : this->map.tiles) {
				if (tile2.regions[0] == tile.regions[0] && tile2.tileType == TileType::INDUSTRIAL && tile2.storedGoods > 0) {
					while (tile2.storedGoods > 0 && receivedGoods != tile.tileVariant + 1) {
						--tile2.storedGoods;
						++receivedGoods;
						industrialRevenue += 100 * (1.0 - this->industrialTax);
					}
